...
{
	template <>
	struct hash<QYHS::QyhsModel::Vertex>
	{
		size_t operator()(QYHS::QyhsModel::Vertex const & vertex) const
		{
			size_t seed = 0;
			QYHS::hashCombine(seed, vertex.position, vertex.color, vertex.normal, vertex.uv);
			return seed;
		}
	};
}
namespace QYHS
{
	...
	QyhsModel::QyhsModel(QyhsDevice & device, const QyhsModel::Builder & builder)
		:qyhsDevice(device)
	{
	...
	void QyhsModel::draw(VkCommandBuffer commandBuffer)
	{
		if (hasIndexBuffer)
		{
	...
	void QyhsModel::createVertexBuffers(const std::vector<Vertex>& vertices)
	{
	...
	{
		std::vector<VkVertexInputBindingDescription> bindDescription(1);
		bindDescription[0].binding = 0;
		bindDescription[0].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;
		bindDescription[0].stride = sizeof(Vertex);

		return bindDescription;
	}
	std::vector<VkVertexInputAttributeDescription> QyhsModel::Vertex::getAttributeDescriptions()
	{
